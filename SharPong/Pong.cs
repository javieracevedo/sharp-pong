...
namespace SharPong
{
    public class SharPong : Game
    {
        ...
        // Flags
        public bool ShowCollisionRect = false;
		...
		private void drawStartScreen() {
			spriteBatch.DrawString(startScreenFont, pongTitle, new Vector2((int)(SCREEN_WIDTH/2) - startScreenFont.MeasureString(pongTitle).X / 2, 100), Color.White);
			spriteBatch.DrawString(startScreenFontSmall, pressStart, new Vector2((int)(SCREEN_WIDTH / 2) - startScreenFontSmall.MeasureString(pressStart).X / 2, 250), Color.White);
		}
		...
		private void drawMainGame(){
            ...
            // Draw ball rect texture/sprite (Collision Box)
            /*if (ShowCollisionRect)
            {
                Color[] ballrect_data = new Color[ballRect.Width * ballRect.Height];
                for (int i = 0; i < ballrect_data.Length; ++i) ballrect_data[i] = Color.DarkSlateGray;
                ballRectTexture.SetData(ballrect_data);
                spriteBatch.Draw(ballRectTexture, ballRect, Color.White);

                Color[] paddleRectData = new Color[paddleOneRect.Width * paddleOneRect.Height];
                for (int i = 0; i < paddleRectData.Length; ++i) paddleRectData[i] = Color.DarkSlateGray;
                paddleOneRectTexture.SetData(paddleRectData);
                paddleTwoRectTexture.SetData(paddleRectData);

                spriteBatch.Draw(paddleOneRectTexture, paddleOneRect, Color.White);
                spriteBatch.Draw(paddleTwoRectTexture, paddleTwoRect, Color.White);

                Color[] horizontalLineRectData = new Color[horizontalLineTopRect.Width * horizontalLineTopRect.Height];
                for (int i = 0; i < horizontalLineRectData.Length; ++i) horizontalLineRectData[i] = Color.DarkSlateGray;
                horizontalLineRectTopTexture.SetData(horizontalLineRectData);
                horizontalLineRectBottomTexture.SetData(horizontalLineRectData);

                spriteBatch.Draw(horizontalLineRectTopTexture, horizontalLineTopRect, Color.White);
                spriteBatch.Draw(horizontalLineRectBottomTexture, horizontalLineBottomRect, Color.White);
            }*/
